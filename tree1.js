...
$('#tree').append('Anfrage verarbeiten<br/>');
  ...
  if (this.readyState == 4 && this.status == 200) {
    ...
    //$('#tree').append(myObj);
    //document.getElementById("#tree").innerHTML = myObj.name;
  //console.log(myObj);
$('#tree').empty();
$('#tree').append('<ul>');
...
//	console.log("test");
	//$.get( "search.php?id="+f+"&msgnr="+ $(this).parent().attr('messageid') +"&marked="+markedstatus, function(d){alert(d);});
    // code block
    break;
  	default:
    // code block
...
function hideNext (test, column){
	...
	//hideNext($(test).parent().next());

}
function showNext (test, column){
	//console.log($(test).find('.toggle'));
...
function buildTree(myObj, margin){
	jQuery.each(myObj.children, function (d, val){
		var marked = val.markedstatus != '0' ? "fas " : "far";
		...
		//alert(new Date(val.date).toLocaleDateString('de-DE') );
