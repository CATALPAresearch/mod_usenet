function search(e) {
  ...
  $("#treeinfo").load(uri, function(responseTxt, statusTxt, xhr) {
    if (statusTxt == "OK") {
...
function navigatePrevius() {
  if ($('#previusbutton').hasClass('disabled')) {
    return;
  }
...
function answerButton() {
  ...
  if (z.indexOf("Antworten") >= 0) {
    ...
    let searchParams = new URLSearchParams(window.location.search);
    let param = searchParams.get('id');
    $.post("posttest.php?id=" + param + "&msgnr=" + $('#messagehead').attr('messageid'), {
      userInput: text,
      subject: subject
    }, function(data) {});
...
function sendButton() {
  ...
  let param = searchParams.get('id');
  var text = $('#messagebody').val();
  $.post("posttest.php?id=" + param + "&msgnr=new", {
    userInput: text,
    subject: subject
